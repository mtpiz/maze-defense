...
import { bodiesOverlap, fitsCorridor, moveGroundCrowd, type CrowdBody } from './ground-crowd.js';
...
describe('Ground crowd occupancy', () => {
  ...
  it('waits without oscillating sideways when a heavy leaves no passing gap', () => {
    const bodies: CrowdBody[] = [
      { id: 'creep-1', x: 0, y: 1000, radius: 80, weight: 1, speed: 1200,
        pattern: 'swarm', lane: 0, route: [4, 5, 6, 7], routeIndex: 0 },
      { id: 'creep-2', x: 600, y: 1000, radius: 450, weight: 20, speed: 0,
        pattern: 'heavy', lane: 0, route: [4, 5, 6, 7], routeIndex: 0 },
    ];
    ...
    const run = (weight: number) => {
      const bodies: CrowdBody[] = [
        { id: 'creep-1', x: 0, y: 0, radius: 110, weight: 8, speed: 1200,
          pattern: 'runner', lane: 0, route: [0, 1, 2, 3], routeIndex: 0 },
        { id: 'creep-2', x: 170, y: 160, radius: 110, weight, speed: 0,
          pattern: 'heavy', lane: 0, route: [0, 1, 2, 3], routeIndex: 0 },
      ];
  ...
  it('clears opposing clumps at a reversing waypoint without phasing or deadlocking', () => {
    const base = definition();
